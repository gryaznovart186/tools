...
func (c *Client) CreateAlgoOrder(ctx context.Context, req *models.CreateAlgoOrderRequest) (*models.AlgoOrder, error) {
	...
	params.Set("algoType", string(req.AlgoType))
	...
	if req.TriggerPrice != nil {
		params.Set("triggerPrice", fmt.Sprintf("%f", *req.TriggerPrice))
	}
	if req.TimeInForce != "" {
...
func (c *Client) GetAlgoOrder(ctx context.Context, algoID int64) (*models.AlgoOrder, error) {
	if err := c.checkCredentials(); err != nil {
		return nil, err
	}
	params := url.Values{}
	params.Set("algoId", fmt.Sprintf("%d", algoID))
...
func (c *Client) OpenAlgoOrders(ctx context.Context, symbol string) ([]models.AlgoOrder, error) {
	if err := c.checkCredentials(); err != nil {
		return nil, err
	}
	params := url.Values{}
...
func (c *Client) CancelAlgoOrder(ctx context.Context, algoID int64) error {
	...
	resp, err := c.rc.R().
		SetContext(ctx).
		SetHeader("X-MBX-APIKEY", c.creds.apiKey).
		Delete("/fapi/v1/algoOrder?" + fullQuery)
